...
#### Initialize model
model = super_cricavpr.Super_CricaVPR(
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    for images, place_id in tqdm(ds):   
        ...
        global_descriptors = model(images.to(args.device))
        global_descriptors = global_descriptors.cuda()
        loss_global = loss_function(global_descriptors, labels) # Call the loss_function we defined above     
        loss = loss_global
    ...
    logging.info(f"Finished epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, "
                 f"average epoch triplet loss = {epoch_losses_global.mean():.4f}")
